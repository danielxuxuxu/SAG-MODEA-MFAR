...
'''图片保存的路径'''
figures_dir = r'E:\SAG_MOEAD_exp_fig\SAG_MOPSO_2D\fig'
...
"""Communication system environment parameters"""
...
def update_bd_prob_velocity(p_bd_prob_list, pbest_bd_prob_list, gbest_bd_prob_list, v_prob_list, r1, r2):
    cognitive_gap = [a - b for a, b in zip(pbest_bd_prob_list, p_bd_prob_list)]
    social_gap = [a - b for a, b in zip(gbest_bd_prob_list, p_bd_prob_list)]
    cognitive_velocity = [x * c1 * r1 for x in cognitive_gap]
    social_velocity = [x * c2 * r2 for x in social_gap]
    v_self_w = [x * w for x in v_prob_list]
    new_v = [a + b + c for a, b, c in zip(v_self_w, cognitive_velocity, social_velocity)]
    return new_v



def update_p_position(position_list_last, velocity_new_n):
    
    p_uav_xy_list_new = []
    for uu in range(Num_UAV):
        p_uav_xy = position_list_last[0][u]
        v_uav_xy_new = velocity_new_n[0][u]
        p_uav_xy_new = [x + y for x, y in zip(p_uav_xy, v_uav_xy_new)]
        p_uav_xy_list_new.append(p_uav_xy_new)
    
    p_iot_bd_AB = position_list_last[1]
    v_iot_bd_AB_new = velocity_new_n[1]
    p_iot_bd_AB_new = [x + y for x, y in zip(p_iot_bd_AB, v_iot_bd_AB_new)]
    
    p_iot_bd_A = position_list_last[2]
    v_iot_bd_A_new = velocity_new_n[2]
    p_iot_bd_A_new = [x + y for x, y in zip(p_iot_bd_A, v_iot_bd_A_new)]
    
    p_iot_bd_B = position_list_last[3]
    v_iot_bd_B_new = velocity_new_n[3]
    p_iot_bd_B_new = [x + y for x, y in zip(p_iot_bd_B, v_iot_bd_B_new)]
...
if __name__ == '__main__':
    ...
    """--------------------------------------Example Initialize N UAV classes----------------------------------------------"""
    ...
    """-------------------------------------Example Initialize N IOT A class lists-----------------------------------------------"""
    
    IOT_class_list_A_o = []
    
    iot_x_list_A_u = load_to_list(r'E:\SAG_MOEAD_exp_fig\IOT_position_u\iot_x_list_A_u.npy')
    iot_y_list_A_u = load_to_list(r'E:\SAG_MOEAD_exp_fig\IOT_position_u\iot_y_list_A_u.npy')
    iot_z_list_A_u = load_to_list(r'E:\SAG_MOEAD_exp_fig\IOT_position_u\iot_z_list_A_u.npy')
    iot_rate_list_A_u = []
    ...
    iot_x_list_B_u = load_to_list(r'E:\SAG_MOEAD_exp_fig\IOT_position_u\iot_x_list_B_u.npy')
    iot_y_list_B_u = load_to_list(r'E:\SAG_MOEAD_exp_fig\IOT_position_u\iot_y_list_B_u.npy')
    iot_z_list_B_u = load_to_list(r'E:\SAG_MOEAD_exp_fig\IOT_position_u\iot_z_list_B_u.npy')
    iot_rate_list_B_u = []
    ...
    for n in range(N):
        ...
        """Initialization Calculates the channel capacity of the initial IOT uplink channel in the NTH individual (solved by two-layer SNR)"""
    ...
    save_to_file(Objective_PF_o, r'E:\SAG_MOEAD_exp_fig\SAG_MOPSO_2D\file\PF_o')
    ...
    save_to_file(IOT_capacity_population_A_PF_o,
                 r'E:\SAG_MOEAD_exp_fig\SAG_MOPSO_2D\file\IOT_capacity_population_A_PF_o')
    save_to_file(IOT_capacity_population_B_PF_o,
                 r'E:\SAG_MOEAD_exp_fig\SAG_MOPSO_2D\file\IOT_capacity_population_B_PF_o')
    ...
    HV_list = []
    """-------------------------------------Initializes the position of the particle---------------------------------------------"""
    Position_list = init_solution_population
    ...
    for pp in range(N):
        ...
        v_bd_prob_B_in = []
        for u in range(Num_UAV):
            vx = random.uniform(0, 0.2 * Length_UAV_area)
            vy = random.uniform(0, 0.2 * Width_UAV_area)
            v_uav_xy.append([vx, vy])
        _, v_bd_prob_AB = generate_iot_bd_proportion(Num_IOT_category)
        v_bd_prob_AB = [x * 0.2 for x in v_bd_prob_AB]
        _, v_bd_prob_A_in = generate_iot_bd_proportion(Num_IOT_A)
        v_bd_prob_A_in = [x * 0.2 for x in v_bd_prob_A_in]
        _, v_bd_prob_B_in = generate_iot_bd_proportion(Num_IOT_B)
        v_bd_prob_B_in = [x * 0.2 for x in v_bd_prob_B_in]
        Velocity_list.append([v_uav_xy, v_bd_prob_AB, [v_bd_prob_A_in, v_bd_prob_B_in]])

    """-------------------------------------Initialize the optimal position for each particle---------------------------------------------"""
    P_best_position = Position_list
    """------------------------------Initializes the historical best target value for each particle and the target value in the current iteration--------------------------------------"""
    P_best_Objective = [list(x) for x in objective_list_combine_o]
    P_Objective = [list(x) for x in objective_list_combine_o]
    """------------------------------------Initializes the optimal position of the global particle---------------------------------------------"""
    G_best_position_id_o = random.choice(PF_index_o)
    G_best_position = Position_list[G_best_position_id_o]
    ...
    for epoch in range(Evolution_epoch):
        ...
        for n in range(N):
            ...
            "Update the speed of each particle - the speed of the uav"
            
            V_uav_xy = []
            for u in range(Num_UAV):
                p_gbest_x_u = G_best_position[0][u][0]
                p_gbest_y_u = G_best_position[0][u][1]
                p_pbest_x_u = P_best_position[n][0][u][0]
                p_pbest_y_u = P_best_position[n][0][u][1]
                p_p_x_u = Position_list[n][0][u][0]
                p_p_y_u = Position_list[n][0][u][1]
                v_p_x_u = Velocity_list[n][0][u][0]
                v_p_y_u = Velocity_list[n][0][u][1]

                v_cognitive_x_u = c1 * r1 * (p_pbest_x_u - v_p_x_u)
                v_cognitive_y_u = c1 * r1 * (p_pbest_y_u - p_p_y_u)
                v_social_x_u = c2 * r2 * (p_gbest_x_u - p_p_x_u)
                v_social_y_u = c2 * r2 * (p_gbest_y_u - p_p_y_u)
                
                v_p_x_u = w * v_p_x_u + v_cognitive_x_u + v_social_x_u
                v_p_y_u = w * v_p_y_u + v_cognitive_y_u + v_social_y_u
                V_uav_xy.append([v_p_x_u, v_p_y_u])

            "Update the speed of each particle - the proportional bandwidth allocation of AB"
            p_gbest_bd_AB = G_best_position[1]
            p_pbest_bd_AB = P_best_position[n][1]
            p_p_bd_AB = Position_list[n][1]
            v_p_bd_AB = Velocity_list[n][1]
            v_p_bd_AB = update_bd_prob_velocity(p_p_bd_AB, p_pbest_bd_AB, p_gbest_bd_AB, v_p_bd_AB, r1, r2)

            "Update the speed of each particle - proportional bandwidth allocation within A"
            p_gbest_bd_A = G_best_position[2][0]
            p_pbest_bd_A = P_best_position[n][2][0]
            p_p_bd_A = Position_list[n][2][0]
            v_p_bd_A = Velocity_list[n][2][0]
            v_p_bd_A = update_bd_prob_velocity(p_p_bd_A, p_pbest_bd_A, p_gbest_bd_A, v_p_bd_A, r1, r2)

            "Update the speed of each particle - proportional bandwidth allocation within B"
            p_gbest_bd_B = G_best_position[2][1]
            p_pbest_bd_B = P_best_position[n][2][1]
            p_p_bd_B = Position_list[n][2][1]
            v_p_bd_B = Velocity_list[n][2][1]
            v_p_bd_B = update_bd_prob_velocity(p_p_bd_B, p_pbest_bd_B, p_gbest_bd_B, v_p_bd_B, r1, r2)

            "Get an updated list of velocities per particle"
            velocity_new_n = [V_uav_xy, v_p_bd_AB, [v_p_bd_A, v_p_bd_B]]
            Velocity_list[n] = velocity_new_n

            "----------------Update the position of each particle-----------------"
            p_uav_xy_list_new = []
            for uu in range(Num_UAV):
                p_uav_xy = Position_list[n][0][uu]
                v_uav_xy_new = velocity_new_n[0][uu]
                p_uav_xy_new = [x + y for x, y in zip(p_uav_xy, v_uav_xy_new)]
                p_uav_xy_list_new.append(p_uav_xy_new)
            
            p_iot_bd_AB = Position_list[n][1]
            v_iot_bd_AB_new = velocity_new_n[1]
            p_iot_bd_AB_new = [x + y for x, y in zip(p_iot_bd_AB, v_iot_bd_AB_new)]
            p_iot_bd_AB_new = [x if x > 0 else 0 for x in p_iot_bd_AB_new]
            p_iot_bd_AB_new = [x / sum(p_iot_bd_AB_new) for x in p_iot_bd_AB_new]
            
            p_iot_bd_A = Position_list[n][2][0]
            v_iot_bd_A_new = velocity_new_n[2][0]
            p_iot_bd_A_new = [x + y for x, y in zip(p_iot_bd_A, v_iot_bd_A_new)]
            p_iot_bd_A_new = [x if x > 0 else 0 for x in p_iot_bd_A_new]
            p_iot_bd_A_new = [x / sum(p_iot_bd_A_new) for x in p_iot_bd_A_new]
            
            p_iot_bd_B = Position_list[n][2][1]
            v_iot_bd_B_new = velocity_new_n[2][1]
            p_iot_bd_B_new = [x + y for x, y in zip(p_iot_bd_B, v_iot_bd_B_new)]
            p_iot_bd_B_new = [x if x > 0 else 0 for x in p_iot_bd_B_new]
            p_iot_bd_B_new = [x / sum(p_iot_bd_B_new) for x in p_iot_bd_B_new]

            Position_new = [p_uav_xy_list_new, p_iot_bd_AB_new, [p_iot_bd_A_new, p_iot_bd_B_new]]
            Position_list[n] = Position_new

            "----------------Calculate the target value of the new particle and update it-----------------"
            Objective_new, IOT_capacity_A_new, IOT_capacity_B_new = calculate_new_solution(
                p_uav_xy_list_new, p_iot_bd_AB_new, p_iot_bd_A_new, p_iot_bd_B_new)
            
            P_Objective[n] = Objective_new
            "----------------Update the current particle's optimal position pbest and the corresponding best objective-----------------"
            if is_dominate(Objective_new, P_best_Objective[n]):
                P_Objective[n] = Objective_new
                P_best_position[n] = Position_new
            else:
                P_best_id = random.choice([0, 1])
                P_Objective[n] = [Objective_new, P_Objective[n]][P_best_id]
                P_best_position[n] = [Position_new, P_best_position[n]][P_best_id]

            "----------------Update global EP and PF-----------------"
            EP.append(Position_new)
        ...
        "----------------------------------------Calculate the HV value of the EP obtained under each epoch---------------------------------"
        Ref_point = [0, 0]
        ...
        Objective_PF, PF_index = pareto_frontier(P_Objective)
        G_best_position_id = random.choice(PF_index)
        G_best_position = Position_list[G_best_position_id]

        "----------------EP and FY are stored-----------------"
        ...
        save_to_file(IOT_capacity_avg_dev_list_A,
                     r'E:\SAG_MOEAD_exp_fig\SAG_MOPSO_2D\file\IOT_capacity_avg_dev_list_A')
        ...
        save_to_file(IOT_capacity_avg_dev_list_B,
                     r'E:\SAG_MOEAD_exp_fig\SAG_MOPSO_2D\file\IOT_capacity_avg_dev_list_B')

        
        plot_pareto_frontier_save(PF_last_epoch, FY, 'No.' + str(epoch + 1) + 'Round iteration of the parent frontier')
        
        PF_last_epoch = FY

        if epoch == Evolution_epoch - 1:
            
            save_to_file(EP, r'E:\SAG_MOEAD_exp_fig\SAG_MOPSO_2D\file\EP_final')
            
            save_to_file(FY, r'E:\SAG_MOEAD_exp_fig\SAG_MOPSO_2D\file\PF_final')
            
            save_to_file(EP_IOT_A_capacity_list,
                         r'E:\SAG_MOEAD_exp_fig\SAG_MOPSO_2D\file\EP_IOT_A_capacity_list_final')
            save_to_file(EP_IOT_B_capacity_list,
                         r'E:\SAG_MOEAD_exp_fig\SAG_MOPSO_2D\file\EP_IOT_B_capacity_list_final')
            ...
            EP_IOT_A_capacity_list_df.to_excel(
                r'E:\SAG_MOEAD_exp_fig\SAG_MOPSO_2D\file\EP_IOT_A_capacity_list_final.xlsx', engine='openpyxl')
            EP_IOT_B_capacity_list_df.to_excel(
                r'E:\SAG_MOEAD_exp_fig\SAG_MOPSO_2D\file\EP_IOT_B_capacity_list_final.xlsx', engine='openpyxl')
            save_to_file(HV_list, r'E:\SAG_MOEAD_exp_fig\SAG_MOPSO_2D\file\HV_list')
