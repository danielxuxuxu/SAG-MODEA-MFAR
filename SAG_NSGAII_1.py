...
figures_dir = r'D:\SAG_MOEAD_exp_fig\SAG_NSGAII_8.0\fig'
...
def crosser_mutation_bd_prop(p1, p2):
    alpha = 0.5
    child1 = [x * alpha for x in p1]
    child2 = [x * (1-alpha) for x in p2]
    child = [x + y for x, y in zip(child1, child2)]
    for pi in range(len(child)):
        if random.random() < bd_prop_mutation_rate:
            mutation_base = child[pi]
            mutation_bd = random.uniform(-mutation_base, mutation_base)
            child[pi] += mutation_bd
    
    nor_child = [x / sum(child) for x in child]
    return nor_child
...
def get_new_solution_new(last_solutions):
    
    
    idx = random.sample(range(0, Neighbor_num - 1), 2)
    
    parents1_idx = idx[0]
    parents2_idx = idx[1]

    uav_xy_p1 = last_solutions[parents1_idx][0]
    uav_xy_p2 = last_solutions[parents2_idx][0]
    uav_xy_cross, _ = crossover(uav_xy_p1, uav_xy_p2)
    uav_xy_list_muta = mutation_UAV_xy(uav_xy_cross, UAV_mutation_rate, Coordinate_UAV_min, Coordinate_UAV_max)

    iot_bd_prop_AB_p1 = last_solutions[parents1_idx][1]
    iot_bd_prop_AB_p2 = last_solutions[parents2_idx][1]
    iot_bd_prop_AB_i_muta = crosser_mutation_bd_prop(iot_bd_prop_AB_p1, iot_bd_prop_AB_p2)

    iot_bd_prop_A_p1 = last_solutions[parents1_idx][2][0]
    iot_bd_prop_A_p2 = last_solutions[parents2_idx][2][0]
    iot_bd_prop_B_p1 = last_solutions[parents1_idx][2][1]
    iot_bd_prop_B_p2 = last_solutions[parents2_idx][2][1]
    iot_bd_prop_a_new = crosser_mutation_bd_prop(iot_bd_prop_A_p1, iot_bd_prop_A_p2)
    iot_bd_prop_b_new = crosser_mutation_bd_prop(iot_bd_prop_B_p1, iot_bd_prop_B_p2)

    return [uav_xy_list_muta, iot_bd_prop_AB_i_muta, [iot_bd_prop_a_new, iot_bd_prop_b_new]]
...
def dominate(p1, p2):
    ...
    return all(p1[i] <= p2[i] for i in range(len(p1))) and any(p1[i] < p2[i] for i in range(len(p1)))


def fast_non_dominated_sort(objective_values_list):
    ...
    for i in range(n):
        for j in range(n):
            if i != j:
                if dominate(objective_values_list[i], objective_values_list[j]):
                    dominates[i].append(j)
                elif dominate(objective_values_list[j], objective_values_list[i]):
                    dominated_count[i] += 1
...
def calculate_crowding_distance_for_front(objective_values_list):
    n = len(objective_values_list)
    distances = [0.0] * n
    ...
    num_objectives = len(objective_values_list[0])
    normalized_values = [[0.0] * num_objectives for _ in range(n)]

    for obj_index in range(num_objectives):
        sorted_indices = sorted(range(n), key=lambda x: objective_values_list[x][obj_index])

        min_value = objective_values_list[sorted_indices[0]][obj_index]
        max_value = objective_values_list[sorted_indices[-1]][obj_index]

        if max_value - min_value == 0:
            continue

        for i in range(n):
            normalized_values[sorted_indices[i]][obj_index] = (objective_values_list[sorted_indices[i]][obj_index] - min_value) / (max_value - min_value)

    for i in range(num_objectives):
        sorted_indices = sorted(range(n), key=lambda x: normalized_values[x][i])

        distances[sorted_indices[0]] = distances[sorted_indices[-1]] = float('inf')

        for j in range(1, n - 1):
            distances[sorted_indices[j]] += normalized_values[sorted_indices[j + 1]][i] - normalized_values[sorted_indices[j - 1]][i]

    return distances


def calculate_crowding(objective_values_list, fronts_index_list):
    crowding_distance_list = []
    for f in range(len(fronts_index_list)):
        front_idx_list = fronts_index_list[f]
        objective_values_list_front = [objective_values_list[i] for i in front_idx_list]
        
        crowding_distance_front = calculate_crowding_distance_for_front(objective_values_list_front)
        crowding_distance_list.append(crowding_distance_front)
    return crowding_distance_list


def select_new_parents(keep_population_size, population, fronts, crowding_distances):
    selected_parents = []
    current_size = 0
    front_index = 0

    while current_size + len(fronts[front_index]) <= keep_population_size:
        selected_parents.extend(fronts[front_index])
        current_size += len(fronts[front_index])
        front_index += 1

    if current_size < keep_population_size:
        remaining_parents = keep_population_size - current_size
        front = fronts[front_index]
        front_crowding_distances = crowding_distances[front_index]
        sorted_indices = sorted(range(len(front)), key=lambda x: front_crowding_distances[x], reverse=True)
        selected_parents.extend([front[i] for i in sorted_indices[:remaining_parents]])

    return [population[i] for i in selected_parents], selected_parents


def unique_rows_with_indices(input_list):
    unique_rows = []
    index_mapping = {}

    for index, row in enumerate(input_list):
        row_tuple = tuple(row)
        if row_tuple not in index_mapping:
            index_mapping[row_tuple] = index
            unique_rows.append(row)

    unique_indices = list(index_mapping.values())
...
if __name__ == '__main__':
    ...
    """-------------------------------------Initialize N IOT A class lists-----------------------------------------------"""
    ...
    save_to_file(Objective_PF_o, r'D:\SAG_MOEAD_exp_fig\SAG_NSGAII_8.0\file\PF_o')
    ...
    save_to_file(IOT_capacity_population_A_PF_o,
                 r'D:\SAG_MOEAD_exp_fig\SAG_NSGAII_8.0\file\IOT_capacity_population_A_PF_o')
    save_to_file(IOT_capacity_population_B_PF_o,
                 r'D:\SAG_MOEAD_exp_fig\SAG_NSGAII_8.0\file\IOT_capacity_population_B_PF_o')
    ...
    Last_object_list = [list(item) for item in Last_object_list]
    ...
    for epoch in range(Evolution_epoch):
        ...
        for i in range(N):
            ...
            New_solution = get_new_solution_new(Last_solution_list)
            
            UAV_xy_new = New_solution[0]
            IOT_bd_cate_prop_list_new = New_solution[1]
            IOT_a_bd_prop_list_new = New_solution[2][0]
            IOT_b_bd_prop_list_new = New_solution[2][1]
            Objective_list_new, IOT_new_cap_A, IOT_new_cap_B = calculate_new_solution(
                UAV_xy_new, IOT_bd_cate_prop_list_new, IOT_a_bd_prop_list_new, IOT_b_bd_prop_list_new)
            
            Last_solution_list.append(New_solution)
            Last_object_list.append(Objective_list_new)
            last_IOT_capacity_A.append(IOT_new_cap_A)
            last_IOT_capacity_B.append(IOT_new_cap_B)

        """The solutions in the solution set are sorted by the front edge"""
        
        Fronts = fast_non_dominated_sort(Last_object_list)
        Fronts = Fronts[:-1]
        """Calculate the crowding value of each layer's front and form a list"""
        Crowding_distance_list = calculate_crowding(Last_object_list, Fronts)
        """Screen out new parent groups"""
        Selected_solution, Selected_index = select_new_parents(N, Last_solution_list, Fronts, Crowding_distance_list)
        """The solution of the first layer front in the population is added to the external populations EP and FY"""
        
        Solution_front_1 = [Last_solution_list[i] for i in Fronts[0]]
        Obj_front_1 = [Last_object_list[i] for i in Fronts[0]]
        last_IOT_capacity_A_f1 = [last_IOT_capacity_A[i] for i in Fronts[0]]
        last_IOT_capacity_B_f1 = [last_IOT_capacity_B[i] for i in Fronts[0]]

        EP += Solution_front_1
        FY += Obj_front_1
        EP_IOT_A_capacity_list += last_IOT_capacity_A_f1
        EP_IOT_B_capacity_list += last_IOT_capacity_B_f1
        """Screening out new parent populations and updating the parent populations"""
        Last_solution_list = Selected_solution
        Last_object_list = [Last_object_list[o] for o in Selected_index]
        last_IOT_capacity_A = [last_IOT_capacity_A[o] for o in Selected_index]
        last_IOT_capacity_B = [last_IOT_capacity_B[o] for o in Selected_index]
        """Update EP: The first layer front in the retained population in each round is judged for dominance, the EP is dominated by the new solution is removed, and if the EP does not dominate F (y'), the new solution is added to the EP"""
        
        FY, unique_index = unique_rows_with_indices(FY)
        EP = [EP[index] for index in unique_index]
        EP_IOT_A_capacity_list = [EP_IOT_A_capacity_list[index] for index in unique_index]
        EP_IOT_B_capacity_list = [EP_IOT_B_capacity_list[index] for index in unique_index]
        ...
        save_to_file(IOT_capacity_avg_dev_list_A,
                     r'D:\SAG_MOEAD_exp_fig\SAG_NSGAII_8.0\file\IOT_capacity_avg_dev_list_A')
        ...
        save_to_file(IOT_capacity_avg_dev_list_B,
                     r'D:\SAG_MOEAD_exp_fig\SAG_NSGAII_8.0\file\IOT_capacity_avg_dev_list_B')
        ...
        if epoch == Evolution_epoch - 1:
            ...
            save_to_file(EP, r'D:\SAG_MOEAD_exp_fig\SAG_NSGAII_8.0\file\EP_final')
            
            save_to_file(FY, r'D:\SAG_MOEAD_exp_fig\SAG_NSGAII_8.0\file\PF_final')
            
            save_to_file(EP_IOT_A_capacity_list,
                         r'D:\SAG_MOEAD_exp_fig\SAG_NSGAII_8.0\file\EP_IOT_A_capacity_list_final')
            save_to_file(EP_IOT_B_capacity_list,
                         r'D:\SAG_MOEAD_exp_fig\SAG_NSGAII_8.0\file\EP_IOT_B_capacity_list_final')
            ...
            EP_IOT_A_capacity_list_df.to_excel(
                r'D:\SAG_MOEAD_exp_fig\SAG_NSGAII_8.0\file\EP_IOT_A_capacity_list_final.xlsx', engine='openpyxl')
            EP_IOT_B_capacity_list_df.to_excel(
                r'D:\SAG_MOEAD_exp_fig\SAG_NSGAII_8.0\file\EP_IOT_B_capacity_list_final.xlsx', engine='openpyxl')
            save_to_file(HV_list, r'D:\SAG_MOEAD_exp_fig\SAG_NSGAII_8.0\file\HV_list')
